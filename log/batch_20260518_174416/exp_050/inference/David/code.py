...
def get_bid(day_context, my_status, opponents_status):
    ...
    # First day strategy: Bid 50% of salary to balance budget and water security.
    # Using int() casting as required by rules.
    bid_amount = float(DAILY_SALARY * 0.5)
    
    if my_status['hp'] <= 2:
        bid_amount = float(DAILY_SALARY * 0.85)
        
    final_bid = min(float(my_status['budget']), bid_amount)
...
def get_bid(day_context, my_status, opponents_status):
    ...
    # Calculate competitive threshold based on previous bids
    yesterday_bids = [o['previous_trace']['bid'] for o in alive_opponents if o.get('previous_trace') and o['previous_trace'].get('bid') is not None]
    avg_prev = sum(yesterday_bids) / len(yesterday_bids) if yesterday_bids else DAILY_SALARY * 1.2
    
    # Strategy: Aggressive if supply is low, conservative if supply is high
    if supply < 18:
        bid = min(my_status['budget'], avg_prev * 1.05)
    elif supply > 22:
        bid = min(my_status['budget'], DAILY_SALARY * 0.8)
    else:
        bid = min(my_status['budget'], avg_prev)
        
    # Desperation mode
    if my_status['hp'] <= 2:
        return min(my_status['budget'], DAILY_SALARY * 2.0)
...
def get_bid(day_context, my_status, opponents_status):
    ...
    if my_status['hp'] <= 3:
        bid = min(my_status['budget'], max(avg_prev_bid * 1.2, 90.0))
    else:
...
def get_bid(day_context, my_status, opponents_status):
    ...
    return min(my_status['budget'], max(avg_prev * 0.8, 35.0))
"""
...
def get_bid(day_context, my_status, opponents_status):
    ...
    if supply < 18:
        return min(my_status['budget'], max(avg_prev * 0.95, DAILY_SALARY * 0.7))
...
def get_bid(day_context, my_status, opponents_status):
    ...
    if supply < 18:
        return min(my_status['budget'], max(DAILY_SALARY * 0.6, max_prev + 2))
