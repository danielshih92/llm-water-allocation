...
DEFAULT_INPUT_DIR = (
    PROJECT_ROOT / "data" / "paper_intermediates" / "failure_analysis"
)
DEFAULT_LOG_DIR = PROJECT_ROOT / "data" / "paper_intermediates" / "logs"
EXP_PATTERN = re.compile(r"^exp_(\d+)$")


def manifest_path(value: Path) -> str:
...
def main() -> None:
    ...
    manifest = {
        ...
        "input_dir": manifest_path(input_dir),
        "output_dir": manifest_path(output_dir),
        "log_dir": manifest_path(args.log_dir),
        "batches": batch_names,
