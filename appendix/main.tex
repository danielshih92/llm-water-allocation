...
These properties make WAC a useful testbed for evaluating whether language-model agents can produce executable policies that balance survival, budget discipline, opponent modeling, and long-term planning under uncertainty.
...
making, and opponent modeling. This separation prevents a model from
being judged solely by its final outcome and allows us to identify the
behavioral mechanisms associated with success or failure.
...
Opponent modeling measures whether a policy conditions its decisions
on strategically relevant opponent behavior and whether access to
opponent policy code improves that adaptation. The final primary metric
and its operational definition are left unspecified here until the
corresponding evaluation is finalized; entries associated with this
metric are therefore marked as ``--'' in the results table.
...
\begin{table*}[t]
    ...
    rate, and NBA denotes Normalized Bid Adjustment. CFAQ, RS, and NBA
    are reported as percentages. Values marked ``--'' await finalization
    of the opponent-modeling evaluation.}
    \label{tab:capability-diagnostics}
    \begin{tabular}{llrrrrr}
        \toprule
        & & \multicolumn{2}{c}{Long-Term Planning}
        & \multicolumn{2}{c}{Risk-Sensitive Decision Making}
        & \multicolumn{1}{c}{Opponent Modeling} \\
        \cmidrule(lr){3-4}\cmidrule(lr){5-6}\cmidrule(lr){7-7}
        Model & Feedback & CSR $\downarrow$ & CFAQ (\%) $\uparrow$
        & RS (\%) $\uparrow$ & NBA (\%) $\uparrow$ & Metric $\uparrow$ \\
        \midrule
        Gemini 3.5 Flash
            & OF  & 0.091 & 65.6 & 73.1 & \textbf{29.0} & -- \\
            & OPF & 0.080 & 70.6 & 79.2 & \textbf{28.1} & -- \\
            & Avg. & 0.086 & 68.1 & 76.1 & \textbf{28.6} & -- \\
        \addlinespace
        Claude Sonnet 5
            & OF  & \textbf{0.037} & 72.8 & 74.5 & 16.3 & -- \\
            & OPF & \textbf{0.030} & 74.3 & 78.2 & 18.5 & -- \\
            & Avg. & \textbf{0.033} & 73.6 & 76.4 & 17.4 & -- \\
        \addlinespace
        GPT-5.4
            & OF  & 0.054 & \textbf{73.5} & \textbf{81.6} & 22.4 & -- \\
            & OPF & 0.096 & \textbf{76.5} & \textbf{88.7} & 23.1 & -- \\
            & Avg. & 0.075 & \textbf{75.0} & \textbf{85.1} & 22.7 & -- \\
        \addlinespace
        DeepSeek V4 Flash
            & OF  & 0.439 & 66.0 & 72.4 & 13.7 & -- \\
            & OPF & 0.320 & 71.5 & 78.1 & 15.1 & -- \\
            & Avg. & 0.379 & 68.8 & 75.3 & 14.4 & -- \\
        \addlinespace
        GPT-5.4 Nano
            & OF  & 0.608 & 69.9 & 72.9 & 7.7 & -- \\
            & OPF & 0.394 & 69.4 & 79.3 & 11.1 & -- \\
            & Avg. & 0.501 & 69.7 & 76.1 & 9.4 & -- \\
        \bottomrule
    \end{tabular}
\end{table*}
...
The opponent-modeling metric remains to be inserted before the final
analysis. Once available, we will use its OF--OPF difference to test
whether policy transparency improves opponent-conditioned adaptation.
